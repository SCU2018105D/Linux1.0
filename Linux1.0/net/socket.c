...
static int get_fd(struct inode *inode)
{
  ...
  // 文件不在直接返回
  if (!file) return(-1);
  // inode中已经存在对应的文件描述符 // 这里应该可以使用索引加内存偏移直接进行计算的。
 ...
 */
/* socket相关的系统调用，都是根据输入名称进行调用的;根据地址偏移计算对应的参数 */
asmlinkage int
